                            ...
                            <td>
                                <?php if (\App\Tablas\Usuario::esta_logueado()) : ?>
                                    <form action="comentar_articulo.php" method="POST" class="inline">
                                        <input type="hidden" name="articulo_id" value="<?= $producto_id ?>">
                                        <input type="hidden" name="usuario_id" value="<?= $usuario_id ?>">
                                        <button type="submit" onclick="comentar(event, <?= $producto_id ?>, <?= $usuario_id ?>)"
                                            class="focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-4 py-2 mr-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-900"
                                            data-modal-toggle="comentar">Comentar</button>
                                    </form>
                                <?php else : ?>
