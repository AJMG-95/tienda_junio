...
<body>
    ...
    $clases_label = [];
    $clases_input = [];
    $error = ['login' => [], 'passsword' => [], 'password_repeat' => []];

    $clases_label_error = "text-red-700 dark:text-red-500";
    $clases_input_error = "bg-red-50 border-red-500 text-red-900 placeholder-red-700 focus:ring-red-500
                            focus:border-red-500 dark:bg-red-100 dark:border-red-400";
    ...
    if (isset($login, $password, $password_repeat)) {
        ...
        if (preg_match('/[A-Z]/', $password) !== 1 ) {
            $error['password'][] = 'La contraseña debe contener almenos una  mayúscula';
        }
        if (preg_match('/[a-z]/', $password) !== 1 ) {
            $error['password'][] = 'La contraseña debe contener almenos una  minúscula';
        }
        if (preg_match('/[[:digit:]]/',$password) !== 1 ) {
            $error['password'][] = 'La contraseña debe contener almenos un digito';
        }
        if (preg_match('/[[:punct:]]/',$password) !== 1 ) {
            $error['password'][] = 'La contraseña debe contener almenos un signo de puntuación';
        }
        if (mb_strlen($password) < 8) {
            $error['password'][] = 'La contraseña debe contener almenos 8 caracteres';
        }
