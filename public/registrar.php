...
<body>
    ...
    if (isset($login, $password, $password_repeat)) {
        ...
        } elseif (\App\Tablas\Usuario::existe($login, $pdo)) {
        ...
        if (preg_match('/[A-Z]/', $password) !== 1) {
            $error['password'][] = 'La contraseña debe tener almanos una letra mayúscula';
        }

        if (preg_match('/[a-z]/', $password) !== 1) {
            $error['password'][] = 'La contraseña debe tener almenos una letra minúscula';
        }

        if (preg_match('/[[:digit:]]/', $password) !== 1) {
            $error['password'][] = 'La contraseña debe tener almanenos un dígito.';
        }

        if (preg_match('/[[:punct:]]/', $password) !== 1) {
            $error['password'][] = 'La contraseña debe tener almenos un signo de puntuación.';
        }

        if (mb_strlen($password) < 8 ) {
            $error['password'][] = 'La contraseña debe tener almenos 8 caracteres.';
        }
