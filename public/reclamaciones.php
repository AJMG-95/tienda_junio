...
<head>
    ...
    <script>
        function cambiarReclamacion(el, id) {
            el.preventDefault();
            const ocultReclamacion = document.getElementById('ocultReclamacion');
            ocultReclamacion.setAttribute('value', id);
        }

        function cambiarModificarReclamacion(el, id) {
            el.preventDefault();
            const ocultoModificarReclamacion = document.getElementById('ocultoModificarReclamacion');
            ocultoModificarReclamacion.setAttribute('value', id);
        }
    </script>
...
<body>
                    ...
                    <?php foreach ($sent as $fila) : ?>
                        ...
                        <td class="py-4 px-6"><?= hh($fila['fecha_factura']) ?></td>
                            ...
                            <td class="px-6 text-center">
                                ...
                                <form action="/admin/editar_reclamacion.php" method="POST" class="inline">
                                    <input type="hidden" name="id" value="<?= $fila_id ?>">
                                    <button type="submit" onclick="cambiarModificarReclamacion(event, <?= $fila_id ?>)" class="focus:outline-none text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-900" data-modal-toggle="modificar">Editar</button>
                                </form>
                                <form action="/admin/borrar_reclamacion.php" method="POST" class="inline">
                                    <input type="hidden" name="id" value="<?= $fila_id ?>">
                                    <button type="submit" onclick="cambiarReclamacion(event, <?= $fila_id ?>)" class="focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900" data-modal-toggle="reclamacion-borrar">Borrar</button>
                                </form>
                            </td>
                        </tr>
                    <?php endforeach ?>
    ...
    <!-- Esto es para borrar -->
    <div id="reclamacion-borrar" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 md:inset-0 h-modal md:h-full">
        <div class="relative p-4 w-full max-w-md h-full md:h-auto">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                <button type="button" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-800 dark:hover:text-white" data-modal-toggle="reclamacion-borrar">
                ...
                <div class="p-6 text-center">
                    ...
                    <form action="/admin/borrar_reclamacion.php" method="POST">
                        ...
                        <button data-modal-toggle="reclamacion-borrar" type="button" class="text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-gray-200 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">No, cancelar</button>
                    </form>
                </div>
            </div>
        </div>
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="p-6 text-center">
                    ...
                    <form action="/admin/editar_reclamacion.php" method="POST">
                        ...
                        <input id="ocultoModificarReclamacion" type="hidden" name="id">
